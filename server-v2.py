...
def get_user_vector(train,inf,user = 0):
    positive_instances = []
    for (u,i) in train.keys():
        if u  < user and u > inf:
            positive_instances.append((u,i))
        if u  > user :
...
class Server(cSimpleModule):
    ...
    def diffuse_message(self,str,sample = False):
        ...
        if str =='FirstRound':
            ...
            for i in participants:
                ...
                # msg.user_ratings = np.array(get_user_vector(train,inf,num))
            ...
            EV << "***************** we're at round :" << self.number_rounds << "***************"
            for i in participants:
                weights = WeightsMessage(str)
                weights.weights = self.model.get_weights()
                self.send(weights,'sl$o',i)
    ...
    def sampling(self,num_samples = 400):
